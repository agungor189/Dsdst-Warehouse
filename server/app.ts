...
const redactSensitive = (value: unknown, secret: string): unknown => {
  if (typeof value === "string") {
    return secret && value.includes(secret) ? value.split(secret).join("[REDACTED]") : value;
  }
  if (Array.isArray(value)) return value.map((item) => redactSensitive(item, secret));
...
export function createWarehouseApp(config: WarehouseBffConfig) {
  ...
  const forward = async (req: Request, res: Response, method: ProxyMethod, upstreamPath: string, query?: URLSearchParams) => {
  ...
  app.get("/api/orders/:id/pick-plan", (req, res) =>
  ...
  app.post("/api/orders/:id/complete", (req, res) =>
    forward(req, res, "POST", `/orders/${encodeURIComponent(String(req.params.id))}/complete`));
