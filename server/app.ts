...
export function createWarehouseApp(config: WarehouseBffConfig) {
  ...
  const requireLivePanelCapability = (capability: "warehouse:print_labels") => async (_req: Request, res: ExpressResponse, next: NextFunction) => {
    if (configurationFailure(res)) return;
    const token = String(res.locals.sessionToken || "");
    if (!token) return res.status(401).json({ success: false, error: { code: "SESSION_REQUIRED", message: "Oturum açmanız gerekiyor." } });
    ...
    if (!upstream.ok) {
      ...
      if (upstream.status >= 500) return res.status(502).json({ success: false, error: { code: "PANEL_AUTH_UNAVAILABLE", message: "Panel kimlik doğrulaması kullanılamıyor." } });
      return res.status(401).json({ success: false, error: { code: "SESSION_INVALID", message: "Oturum geçersiz veya iptal edilmiş." } });
    }
    const user = (body as { user?: { role?: unknown; permissions?: unknown } }).user;
    ...
    if (!allowed) return res.status(403).json({ success: false, error: { code: "FORBIDDEN", message: `Bu işlem için ${capability} capability gerekli.` } });
  ...
  app.post("/api/labels/preview", requireSession, requireLivePanelCapability("warehouse:print_labels"), async (req, res) => {
    ...
    for (const header of ["x-label-template-id", "x-label-template-purpose"] as const) {
  ...
  const defaultTemplateSnapshot = async (res: ExpressResponse, purpose: "goods_receipt" | "location") => {
  ...
  app.post("/api/inventory/v1/reservations/:id/dispatch", requireSession, (req, res) =>
    res.status(409).json({ success: false, error: { code: "PHYSICAL_HANDOFF_REQUIRED",
  ...
  app.get("/api/shipping/v1/reservations/:id/shipment", requireSession, (req, res) =>
    forward(req, res, "GET", `/shipping/reservations/${encodeURIComponent(String(req.params.id))}/shipment`));
  app.post("/api/shipping/v1/shipments/:id/packages", requireSession, (req, res) => {
    const packages = Array.isArray(req.body?.packages) ? req.body.packages.slice(0, 50).map((item: any) => ({
      ...
      contents: Array.isArray(item?.contents) ? item.contents.slice(0, 200).map((content: any) => ({
  ...
  app.post("/api/shipping/v1/shipments/:id/carrier-selection", requireSession, (req, res) => {
    if (req.body?.cashOnDelivery === true) return res.status(409).json({ success: false, error: { code: "COD_FORBIDDEN", message: "Kapıda ödeme desteklenmiyor." } });
  ...
  app.post("/api/shipping/v1/shipments/:id/booking", requireSession, (req, res) =>
    forward(req, res, "POST", `/shipping/shipments/${encodeURIComponent(String(req.params.id))}/booking`, undefined, {
  ...
  app.post("/api/shipping/v1/shipments/:id/packages/:packageId/print", requireSession, (req, res) =>
    forward(req, res, "POST", `/shipping/shipments/${encodeURIComponent(String(req.params.id))}/packages/${encodeURIComponent(String(req.params.packageId))}/print`, undefined, {
  ...
  app.get("/api/returns/:id", requireSession, (req, res) =>
    forward(req, res, "GET", `/returns/${encodeURIComponent(String(req.params.id))}`));
  app.post("/api/returns/:id/receipts", requireSession, (req, res) =>
    forward(req, res, "POST", `/returns/${encodeURIComponent(String(req.params.id))}/receipts`, undefined, {
      lines: Array.isArray(req.body?.lines) ? req.body.lines.slice(0, 100).map((line: any) => ({
  ...
  app.post("/api/execution/topology", requireSession, (req, res) =>
    forward(req, res, "POST", "/execution/topology", undefined, safeAdminBody(req.body)));
  app.get("/api/execution/settings", requireSession, (req, res) => forward(req, res, "GET", "/execution/settings"));
  app.post("/api/execution/settings", requireSession, (req, res) =>
    forward(req, res, "POST", "/execution/settings", undefined, safeAdminBody(req.body)));
  app.post("/api/execution/receipts/excess-approvals", requireSession, (req, res) =>
    forward(req, res, "POST", "/execution/receipts/excess-approvals", undefined, safeAdminBody(req.body)));
  app.post("/api/execution/receipts", requireSession, (req, res) =>
    forward(req, res, "POST", "/execution/receipts", undefined, safeAdminBody(req.body)));
  app.get("/api/execution/packages/:id", requireSession, (req, res) =>
    forward(req, res, "GET", `/execution/packages/${encodeURIComponent(String(req.params.id))}`));
  app.post("/api/execution/packages/:id/identity", requireSession, (req, res) =>
    forward(req, res, "POST", `/execution/packages/${encodeURIComponent(String(req.params.id))}/identity`, undefined, safeAdminBody(req.body)));
  app.get("/api/execution/packages/:id/suggestion", requireSession, (req, res) =>
    forward(req, res, "GET", `/execution/packages/${encodeURIComponent(String(req.params.id))}/suggestion`));
  app.post("/api/execution/packages/:id/place", requireSession, (req, res) =>
    forward(req, res, "POST", `/execution/packages/${encodeURIComponent(String(req.params.id))}/place`, undefined, safeAdminBody(req.body)));
  app.post("/api/execution/packages/:id/move", requireSession, (req, res) =>
    forward(req, res, "POST", `/execution/packages/${encodeURIComponent(String(req.params.id))}/move`, undefined, safeAdminBody(req.body)));
  app.post("/api/execution/replenishments/prepare", requireSession, (req, res) =>
    forward(req, res, "POST", "/execution/replenishments/prepare", undefined, safeAdminBody(req.body)));
  app.get("/api/execution/replenishments", requireSession, (req, res) =>
    forward(req, res, "GET", "/execution/replenishments"));
  app.post("/api/execution/replenishments/:id/complete", requireSession, (req, res) =>
    forward(req, res, "POST", `/execution/replenishments/${encodeURIComponent(String(req.params.id))}/complete`, undefined, safeAdminBody(req.body)));
  app.post("/api/execution/discrepancies", requireSession, (req, res) =>
    forward(req, res, "POST", "/execution/discrepancies", undefined, safeAdminBody(req.body)));
  app.post("/api/execution/counts", requireSession, (req, res) =>
    forward(req, res, "POST", "/execution/counts", undefined, safeAdminBody(req.body)));
  app.post("/api/execution/counts/:id/approve", requireSession, (req, res) =>
    forward(req, res, "POST", `/execution/counts/${encodeURIComponent(String(req.params.id))}/approve`, undefined, safeAdminBody(req.body)));
  app.get("/api/execution/products/:id/reconciliation", requireSession, (req, res) =>
    forward(req, res, "GET", `/execution/products/${encodeURIComponent(String(req.params.id))}/reconciliation`));
  ...
  app.get("/api/admin/packages/:id/print-preview", requireSession, (req, res) =>
    forward(req, res, "GET", `/admin/packages/${encodeURIComponent(String(req.params.id))}/print-preview`));
  ...
  app.post("/api/admin/print-jobs/:id/reprint", requireSession, (req, res) =>
    forward(req, res, "POST", `/admin/print-jobs/${encodeURIComponent(String(req.params.id))}/reprint`, undefined, {
      reason: safeQueryText(req.body?.reason, 40),
      explanation: safeQueryText(req.body?.explanation, 1000) || null,
      idempotency_key: safeQueryText(req.body?.idempotency_key, 200),
    }));
  app.post("/api/admin/print-jobs/:id/confirm", requireSession, (req, res) =>
    forward(req, res, "POST", `/admin/print-jobs/${encodeURIComponent(String(req.params.id))}/confirm`, undefined, {
      idempotency_key: safeQueryText(req.body?.idempotency_key, 200),
    }));
  app.get("/api/admin/locations", requireSession, (req, res) => forward(req, res, "GET", "/admin/locations"));
  app.get("/api/admin/locations/:id/print-preview", requireSession, (req, res) =>
    forward(req, res, "GET", `/admin/locations/${encodeURIComponent(String(req.params.id))}/print-preview`));
  ...
  app.post("/api/admin/layouts/import-legacy", requireSession, (req, res) =>
    forward(req, res, "POST", "/admin/layouts/import-legacy", undefined, safeAdminBody(req.body)));
