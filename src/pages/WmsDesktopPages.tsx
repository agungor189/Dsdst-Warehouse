import { Activity, Box, Boxes, Eye, MapPin, Printer, RefreshCw, Search, Warehouse } from "lucide-react";
import { useEffect, useState, type FormEvent } from "react";
import { Link } from "react-router-dom";
import { PermissionPage } from "./WarehouseAdminPages";
import { getErrorMessage, labelApi, warehouseAdminApi, warehouseApi } from "../lib/api";
import { locationLabelData, openPdfBlob } from "../lib/labels";
...
export function PackagesPage() {
  ...
  const reprint = async (item: WarehousePackageListItem) => { setPrintingId(item.id); setError(""); try { await warehouseAdminApi.queuePrint(item.id); setMessage(`${item.package_code} etiketi yeniden baskı kuyruğuna eklendi.`); } catch (reason) { setError(getErrorMessage(reason)); } finally { setPrintingId(""); } };
  return <PermissionPage permission="warehouse:view_analytics"><div className="space-y-5"><PageHeader eyebrow="Depo" title="Paketler" description="Büyük veri setleri için Panel tarafında filtrelenen ve sayfalanan paket görünümü."/>
    ...
    {message && <p role="status" className="rounded-2xl bg-emerald-50 p-4 font-bold text-success">{message}</p>}{error && <p role="alert" className="rounded-2xl bg-red-50 p-4 font-bold text-danger">{error}</p>}
    <div className="table-shell"><table className="w-full min-w-[1180px] text-left text-sm"><thead><tr>{["Package Code","SKU","Ürün","Lot","Paket","Adet","Lokasyon","Ağırlık","Status","Yerleştirilme","Etiket"].map((h) => <th key={h}>{h}</th>)}</tr></thead><tbody>{items.map((item) => <tr key={item.id}><td className="font-black">{item.package_code}</td><td>{item.sku}</td><td>{item.product_name}</td><td>{item.lot_number || "—"}</td><td>{item.package_number}/{item.total_packages}</td><td>{item.quantity}</td><td>{item.location_code || "—"}</td><td>{item.weight || 0} kg</td><td>{item.status}</td><td>{formatDate(item.placed_at)}</td><td><button className="secondary-button whitespace-nowrap" disabled={printingId === item.id} onClick={() => void reprint(item)}><Printer size={16}/>Yeniden Yazdır</button></td></tr>)}</tbody></table></div>
...
export function LocationsDesktopPage() {
  ...
  const selectedLocation = snapshot?.locations.find((location) => location.code === selectedCode);
  const preview = async () => { if (!selectedCode) return; setBusy(true); setError(""); try { openPdfBlob(await labelApi.preview("location", locationLabelData(selectedCode))); } catch (reason) { setError(getErrorMessage(reason)); } finally { setBusy(false); } };
  const print = async () => { if (!selectedLocation) return; setBusy(true); setError(""); try { await warehouseAdminApi.queueLocationPrint(selectedLocation.id); setMessage(`${selectedLocation.code} etiketi baskı kuyruğuna eklendi.`); } catch (reason) { setError(getErrorMessage(reason)); } finally { setBusy(false); } };
  return <PermissionPage permission="warehouse:view_map"><div className="space-y-5"><PageHeader eyebrow="Depo" title="Lokasyonlar" description="Kapasite, doluluk ve aktif rezervasyonlar Panel veritabanından hesaplanır."/>{message && <p role="status" className="rounded-2xl bg-emerald-50 p-4 font-bold text-success">{message}</p>}{error && <p role="alert">{error}</p>}<div className="table-shell"><table className="w-full min-w-[760px] text-left text-sm"><thead><tr>{["Lokasyon","Raf","Kat","Pozisyon","Kapasite","Dolu","Rezerve","Boş"].map((h) => <th key={h}>{h}</th>)}</tr></thead><tbody>{snapshot?.locations.map((item) => { const match = item.code.match(/^(.*)-K(\d+)-P(\d+)$/); return <tr key={item.id} className="cursor-pointer" onClick={() => setSelectedCode(item.code)}><td><button className="font-black text-forest underline decoration-moss/40 underline-offset-4">{item.code}</button></td><td>{item.rack_code}</td><td>{match?.[2] || "—"}</td><td>{match?.[3] || "—"}</td><td>{item.capacity}</td><td>{item.occupied}</td><td>{item.reserved}</td><td>{item.available}</td></tr>; })}</tbody></table></div>{selectedCode && <section className="rounded-2xl border border-line bg-white p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="eyebrow">Lokasyon Detayı</p><h2 className="text-xl font-black">{selectedCode}</h2></div><div className="flex gap-2"><button className="secondary-button" disabled={busy} onClick={() => void preview()}><Eye size={17}/>Önizle</button><button className="primary-button" disabled={busy} onClick={() => void print()}><Printer size={17}/>Etiket Yazdır</button><button className="secondary-button" onClick={() => setSelectedCode(null)}>Kapat</button></div></div><div className="mt-4 grid gap-2 md:grid-cols-2">{selectedPackages.map((pkg) => <article className="rounded-xl border border-line p-3" key={pkg.id}><b>{pkg.sku}</b><p className="text-sm text-muted">{pkg.product_name} · {pkg.package_code} · {pkg.quantity} adet</p></article>)}{selectedPackages.length === 0 && <p className="text-sm text-muted">Bu lokasyonda paket yok.</p>}</div></section>}</div></PermissionPage>;
}
